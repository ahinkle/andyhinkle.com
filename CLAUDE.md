...
- **SEO / Head Management**: `laravel/head` renders all `<head>` metadata via the `@head` directive in the app layout. Site-wide defaults (including Person + WebSite JSON-LD) and error-page metadata live in `AppServiceProvider`; pages declare their metadata with the `<x-seo>`, `<x-seo.blog-post>`, and `<x-seo.speaking>` components (`app/View/Components/Seo*`), which emit page JSON-LD (BlogPosting, PodcastEpisode).
