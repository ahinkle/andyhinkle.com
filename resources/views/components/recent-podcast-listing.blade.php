<div class="grid gap-12 gap-y-4 py-4 md:gap-y-8 lg:grid-cols-3 lg:gap-x-4 lg:gap-y-12">
    @foreach ($podcasts as $episode)
        <a class="group rounded-lg bg-gray-900 p-4" href="{{ url("/speaking/{$episode->slug}") }}">
            <div class="grid grid-cols-1 gap-4">
                ...
                <div class="col-span-1">
                    <div class="flex h-full flex-col">
                        ...
                        <div class="mt-3 grow">
                            <p class="line-clamp-3 text-sm text-gray-300">{{ str($episode->summary)->words(20) }}</p>
                        </div>
                            ...
                            <p class="flex items-center justify-between text-xs text-white">
                                <span class="flex items-center">
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 448 512"
                                        class="mr-1 mb-0.5 h-3 w-3 fill-current text-gray-300"
                                    >
                                ...
                                @if ($episode->duration && $episode->type === 'podcast')
                                    <span class="flex items-center">
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            class="mr-1 h-3 w-3 fill-current text-gray-300"
                                            viewBox="0 0 20 20"
                                            fill="currentColor"
                                        >
                                            <path
