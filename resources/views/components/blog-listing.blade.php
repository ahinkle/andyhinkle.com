    ...
    @foreach ($posts as $post)
        <article class="group py-8 first:pt-0">
            <a href="{{ url('/blog/' . $post->slug) }}" class="block">
                ...
                <p class="mt-3 leading-relaxed text-gray-400">{{ $post->description }}</p>
                <span class="mt-4 inline-flex items-center gap-1 text-sm text-gray-500 transition-colors group-hover:text-white">
                    Read article
                    <svg
